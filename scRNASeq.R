...
panel_timecourse.con$findCommunities(method=leiden.community, resolution=1.3) #how to set a seed here???
...
panel_timecourse.con$plotGraph(clustering='leiden', size=1) #or con$plotGraph(clustering='walktrap', size=0.8)
panel_timecourse.con$plotGraph(gene="Mki67") #or con$plotGraph(clustering='walktrap', size=0.8)
...
#PLOTTING
DimPlot(timecourse, reduction = "UMAP", group.by = c("ident"), pt.size=.7,
        cols =c(wes_palette("Darjeeling1"), wes_palette("Cavalcanti1")),
        order = c("T cells", "Tuft cells","Enterocytes 3", "Enterocytes 2", "Enterocytes 1", "Enterocytes (immature)", "Early progenitors", "Stem cells", "Paneth cells", "Goblet cells"))
...
ElbowPlot(object = crypt) #Find significant PCAs; 15 are significant
...
crypt.clusters <- FindClusters(crypt)
...
#on D164A 0d vs 2d
Idents(norm.crypt) <- "orig.ident" #set original identity as active identity 
D164A_0vs2d <- FindMarkers(norm.crypt, ident.1 = "D164A 2d pi", ident.2 = "D164A 0d pi", verbose = FALSE)
ranks <- D164A_0vs2d %>% 
  na.omit()%>%
